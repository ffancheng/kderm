...
\if0\blind
{
  \title{\bf Distortion corrected kernel density estimate on Riemannian manifolds}
...
\if1\blind
{
  ...
  \bigskip
  \begin{center}
    {\LARGE\bf Distortion corrected kernel density estimate on Riemannian manifolds}
  \end{center}
  \medskip
} \fi
...
Multivariate kernel density estimation (KDE see Parzen (\protect\hyperlink{ref-Parzen1962-gt}{1962}); Scott (\protect\hyperlink{ref-Scott2015-vl}{2015}) and references therein) is a fundamental tool for exploratory data analysis and unsupervised learning. Applications of KDE include finding hot spots of traffic networks in the GIS environment (\protect\hyperlink{ref-Xie2008-eb}{Xie and Yan 2008}; \protect\hyperlink{ref-Okabe2009-nb}{Okabe, Satoh, and Sugihara 2009}), automatic detection in visual surveillance systems (\protect\hyperlink{ref-Elgammal2002-cw}{Elgammal et al. 2002}), wind power density detection (\protect\hyperlink{ref-Jeon2012-ac}{Jeon and Taylor 2012}) and prime prediction via Twitter messages (\protect\hyperlink{ref-Gerber2014-tq}{Gerber 2014}). For \(m\)-dimensional data where \(m\) is even of moderate size, KDE is challenging due to the well-known curse of dimensionality. However, the manifold hypothesis suggests that many high-dimensional datasets belong to a \(d\)-dimensional manifold \(M\) where \(d\ll m\). The problem of KDE on manifolds has been well studied in Hendriks (\protect\hyperlink{ref-Hendriks1990-bt}{1990}), Izenman (\protect\hyperlink{ref-Izenman1991-cj}{1991}), Pelletier (\protect\hyperlink{ref-Pelletier2005-vu}{2005}), Henry, Muñoz, and Rodríguez (\protect\hyperlink{ref-Henry2013-da}{2013}), and Berenfeld, Rosa, and Rousseau (\protect\hyperlink{ref-Berenfeld2022-fd}{2022}). However, in these settings, it is assumed that the underlying manifold is known, which is rarely true in practice. Our objective in this paper is to propose KDE for data that lie on some unknown manifold.

When dealing with data that lie on a manifold, it is common to use dimension-reduction techniques. These algorithms which include ISOMAP, LLE, t-SNE, and UMAP among others, can be thought of as a mapping \(\psi:M\rightarrow \mathbb{R}^d\) that take points on the manifold \(\pmb{p}\) to \(d\)-dimensional vectors \(\pmb{y}\). Details of these algorithms are summarized in Cayton (\protect\hyperlink{ref-Cayton2005-dp}{2005}), Lee and Verleysen (\protect\hyperlink{ref-Lee2007-wq}{2007}), Izenman (\protect\hyperlink{ref-Izenman2012-mx}{2012}), and Cheng, Panagiotelis, and Hyndman (\protect\hyperlink{ref-Cheng2021-dh}{2021}). One could easily apply KDE to the output vectors \(\pmb{y}\). However, this would be a misleading representation of the density of points as they lie on the manifold. To see how, consider that the density on the manifold must be defined with respect to an appropriate volume form, that is
...
\begin{algorithm}[!htb]
  ...
  \Output{ low-dimensional data $\pmb{y}_i \in \mathbb{R}^d$ and its Riemannian metric $\pmb{H}_i$ for all $i=1,\ldots,N$ }
  ...
  \STATE Obtain the $d\times d\times N$ tensor $\pmb{\tilde{H}}$ by applying the graph Laplacian $\widetilde{\mathcal{L}}_{\sqrt{\varepsilon},N}$ to the embedding coordinates matrix $\pmb{Y}$ with elements in $\pmb{\tilde{H}}$ found by
  $$
    \pmb{\tilde{H}}^{lk \cdot} = \frac{1}{2} \left[\tilde{\mathcal{L}}_{\varepsilon, N}\left(\pmb{y}^l \odot \pmb{y}^k\right) - \pmb{y}^l \odot\left(\tilde{\mathcal{L}}_{\varepsilon, n} \pmb{y}^k\right) - \pmb{y}^k \odot\left(\tilde{\mathcal{L}}_{\varepsilon, n} \pmb{y}^l\right)\right],
  $$
  where $l,k=1,\dots,d$, $\pmb{\tilde{H}}^{lki}$ is the element in row $l$ and column $k$ of the inverse Riemannian corresponding to observation $i$, and the $\odot$ calculation is the element-wise product between two vectors; 

  \STATE For each embedding data point $\pmb{y}_i, i=1,\dots,N$, calculate its $d\times d$ Riemannian metric $\pmb{H}_i$ as the rank $d$ pseudo inverse of $\tilde{\pmb{H}^{\cdot\cdot i}}$ below 
  $$
    \pmb{H}_i = \pmb{U} \pmb{\Lambda}_d^{-1} \pmb{U}^\prime,
  $$
  where $[\pmb{U}, \pmb{\Lambda}]$ is the eigendecomposition of the $d\times d$ matrix $\tilde{\pmb{H}^{\cdot\cdot i}}$, $\pmb{U}$ is the matrix of column eigenvectors ordered by the eigenvalues in descending order, and $\pmb{\Lambda}_d$ is a diagonal matrix containing the top $d$ largest eigenvalues.
...
The \emph{geodesics} of \(M\) are the smooth curves that locally join the points along the shortest path on the manifold. Intuitively, geodesics are the \emph{straightest possible curves} in a Riemannian manifold (Section 7.2.3 of \protect\hyperlink{ref-Nakahara2018-zs}{Nakahara 2018}).
...
\leavevmode\vadjust pre{\hypertarget{ref-Berenfeld2021-as}{}}%
Berenfeld, Clément, and Marc Hoffmann. 2021. {``Density Estimation on an Unknown Submanifold.''} \emph{European Journal of Sport Science: EJSS: Official Journal of the European College of Sport Science} 15 (1): 2179--2223. \url{https://doi.org/10.1214/21-EJS1826}.

\leavevmode\vadjust pre{\hypertarget{ref-Berenfeld2022-fd}{}}%
Berenfeld, Clément, Paul Rosa, and Judith Rousseau. 2022. {``Estimating a Density Near an Unknown Manifold: A Bayesian Nonparametric Approach,''} May. \url{http://arxiv.org/abs/2205.15717}.

\leavevmode\vadjust pre{\hypertarget{ref-Berry2017-rz}{}}%
Berry, Tyrus, and Timothy Sauer. 2017. {``Density Estimation on Manifolds with Boundary.''} \emph{Computational Statistics \& Data Analysis} 107 (March): 1--17. \url{https://doi.org/10.1016/j.csda.2016.09.011}.

\leavevmode\vadjust pre{\hypertarget{ref-Breiman1977-qc}{}}%
Breiman, Leo, William Meisel, and Edward Purcell. 1977. {``Variable Kernel Estimates of Multivariate Densities.''} \emph{Technometrics: A Journal of Statistics for the Physical, Chemical, and Engineering Sciences} 19 (2): 135--44. \url{https://doi.org/10.1080/00401706.1977.10489521}.

\leavevmode\vadjust pre{\hypertarget{ref-Le_Brigant2019-lj}{}}%
Brigant, Alice le, and Stéphane Puechmorel. 2019. {``Approximation of Densities on Riemannian Manifolds.''} \emph{Entropy} 21 (1). \url{https://doi.org/10.3390/e21010043}.

\leavevmode\vadjust pre{\hypertarget{ref-Cayton2005-dp}{}}%
Cayton, Lawrence. 2005. {``Algorithms for Manifold Learning.''} \emph{Univ. Of California at San Diego Tech. Rep} 12 (1-17): 1.

\leavevmode\vadjust pre{\hypertarget{ref-Chavel2006-mp}{}}%
Chavel, Isaac. 2006. \emph{Riemannian Geometry: A Modern Introduction}. Cambridge University Press.

\leavevmode\vadjust pre{\hypertarget{ref-Cheng2021-dh}{}}%
Cheng, Fan, Anastasios Panagiotelis, and Rob J Hyndman. 2021. {``Computationally Efficient Learning of Statistical Manifolds.''} \emph{arXiv e-Prints}, February, arXiv:2103.11773. \url{https://ui.adsabs.harvard.edu/abs/2021arXiv210311773C}.

\leavevmode\vadjust pre{\hypertarget{ref-cer2012-data}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-Denti2021-qc}{}}%
Denti, Francesco. 2021. {``intRinsic: An {R} Package for Model-Based Estimation of the Intrinsic Dimension of a Dataset,''} February. \url{https://arxiv.org/abs/2102.11425}.

\leavevmode\vadjust pre{\hypertarget{ref-Denti2021-jl}{}}%
Denti, Francesco, Diego Doimo, Alessandro Laio, and Antonietta Mira. 2021. {``Distributional Results for {Model-Based} Intrinsic Dimension Estimators,''} April. \url{https://arxiv.org/abs/2104.13832}.

\leavevmode\vadjust pre{\hypertarget{ref-Elgammal2002-cw}{}}%
Elgammal, A, R Duraiswami, D Harwood, and L S Davis. 2002. {``Background and Foreground Modeling Using Nonparametric Kernel Density Estimation for Visual Surveillance.''} \emph{Proceedings of the IEEE} 90 (7): 1151--63. \url{https://doi.org/10.1109/JPROC.2002.801448}.

\leavevmode\vadjust pre{\hypertarget{ref-Facco2017-rl}{}}%
Facco, Elena, Maria d'Errico, Alex Rodriguez, and Alessandro Laio. 2017. {``Estimating the Intrinsic Dimension of Datasets by a Minimal Neighborhood Information.''} \emph{Scientific Reports} 7 (1): 12140. \url{https://doi.org/10.1038/s41598-017-11873-y}.

\leavevmode\vadjust pre{\hypertarget{ref-Gallot2004-rc}{}}%
Gallot, Sylvestre, Dominique Hulin, and Jacques Lafontaine. 2004. {``Riemannian Metrics.''} In \emph{Riemannian Geometry}, 51--127. Berlin, Heidelberg: Springer Berlin Heidelberg. \url{https://doi.org/10.1007/978-3-642-18855-8/_2}.

\leavevmode\vadjust pre{\hypertarget{ref-Gerber2014-tq}{}}%
Gerber, Matthew S. 2014. {``Predicting Crime Using Twitter and Kernel Density Estimation.''} \emph{Decision Support Systems} 61 (May): 115--25. \url{https://doi.org/10.1016/j.dss.2014.02.003}.

\leavevmode\vadjust pre{\hypertarget{ref-Hendriks1990-bt}{}}%
Hendriks, Harrie. 1990. {``Nonparametric Estimation of a Probability Density on a Riemannian Manifold Using Fourier Expansions.''} \emph{Annals of Statistics} 18 (2): 832--49. \url{http://www.jstor.org/stable/2242136}.

\leavevmode\vadjust pre{\hypertarget{ref-Henry2013-da}{}}%
Henry, Guillermo, Andrés Muñoz, and Daniela Rodríguez. 2013. {``Locally Adaptive Density Estimation on Riemannian Manifolds.''} \emph{SORT-Statistics and Operations Research Transactions} 37 (2): 111--30. \url{https://raco.cat/index.php/SORT/article/view/271165}.

\leavevmode\vadjust pre{\hypertarget{ref-Hyndman2018-ia}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-Izenman1991-cj}{}}%
Izenman, Alan Julian. 1991. {``Review Papers: Recent Developments in Nonparametric Density Estimation.''} \emph{Journal of the American Statistical Association} 86 (413): 205--24. \url{https://doi.org/10.1080/01621459.1991.10475021}.

\leavevmode\vadjust pre{\hypertarget{ref-Izenman2012-mx}{}}%
---------. 2012. {``Introduction to Manifold Learning.''} \emph{WIREs Comp Stat} 4 (5): 439--46.

\leavevmode\vadjust pre{\hypertarget{ref-Jeon2012-ac}{}}%
Jeon, Jooyoung, and James W Taylor. 2012. {``Using Conditional Kernel Density Estimation for Wind Power Density Forecasting.''} \emph{Journal of the American Statistical Association} 107 (497): 66--79. \url{https://doi.org/10.1080/01621459.2011.643745}.

\leavevmode\vadjust pre{\hypertarget{ref-Jones1990-oe}{}}%
Jones, M C. 1990. {``Variable Kernel Density Estimates and Variable Kernel Density Estimates.''} \emph{The Australian Journal of Statistics} 32 (3): 361--71.

\leavevmode\vadjust pre{\hypertarget{ref-Lee2007-wq}{}}%
Lee, John A, and Michel Verleysen. 2007. \emph{Nonlinear Dimensionality Reduction}. Springer Science \& Business Media.

\leavevmode\vadjust pre{\hypertarget{ref-McQueen2016-xz}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-Nakahara2018-zs}{}}%
Nakahara, M. 2018. \emph{{Geometry, topology and physics}}. {CRC press}.

\leavevmode\vadjust pre{\hypertarget{ref-Okabe2009-nb}{}}%
Okabe, Atsuyuki, Toshiaki Satoh, and Kokichi Sugihara. 2009. {``A Kernel Density Estimation Method for Networks, Its Computational Method and a {GIS‐based} Tool.''} \emph{Geographical Information Systems} 23 (1): 7--32. \url{https://doi.org/10.1080/13658810802475491}.

\leavevmode\vadjust pre{\hypertarget{ref-Parzen1962-gt}{}}%
Parzen, Emanuel. 1962. {``On Estimation of a Probability Density Function and Mode.''} \emph{Annals of Mathematical Statistics} 33 (3): 1065--76.

\leavevmode\vadjust pre{\hypertarget{ref-Pelletier2005-vu}{}}%
Pelletier, Bruno. 2005. {``Kernel Density Estimation on Riemannian Manifolds.''} \emph{Statistics \& Probability Letters} 73 (3): 297--304.

\leavevmode\vadjust pre{\hypertarget{ref-Perrault-Joncas2013-pq}{}}%
Perrault-Joncas, Dominique, and Marina Meila. 2013. {``Non-Linear Dimensionality Reduction: Riemannian Metric Estimation and the Problem of Geometric Discovery,''} May. \url{https://arxiv.org/abs/1305.7255}.

\leavevmode\vadjust pre{\hypertarget{ref-Scott2015-vl}{}}%
Scott, David W. 2015. \emph{{Multivariate Density Estimation: Theory, Practice, and Visualization}}. John Wiley \& Sons.

\leavevmode\vadjust pre{\hypertarget{ref-Terrell1992-ut}{}}%
Terrell, George R, and David W Scott. 1992. {``Variable Kernel Density Estimation.''} \emph{Annals of Statistics} 20 (3): 1236--65.

\leavevmode\vadjust pre{\hypertarget{ref-Wickham2011-ir}{}}%
Wickham, Hadley, Dianne Cook, Heike Hofmann, and Andreas Buja. 2011. {``Tourr: An {R} Package for Exploring Multivariate Data with Projections.''} \emph{Journal of Statistical Software} 40 (April): 1--18. \url{https://doi.org/10.18637/jss.v040.i02}.

\leavevmode\vadjust pre{\hypertarget{ref-Xie2008-eb}{}}%
Xie, Zhixiao, and Jun Yan. 2008. {``Kernel Density Estimation of Traffic Accidents in a Network Space.''} \emph{Computers, Environment and Urban Systems} 32 (5): 396--406. \url{https://doi.org/10.1016/j.compenvurbsys.2008.05.001}.

\leavevmode\vadjust pre{\hypertarget{ref-Zhou2011-za}{}}%
Zhou, Xueyuan, and Mikhail Belkin. 2011. {``Semi-Supervised Learning by Higher Order Regularization.''} In \emph{Proceedings of the Fourteenth International Conference on Artificial Intelligence and Statistics}, 15:892--900. Proceedings of Machine Learning Research. JMLR Workshop; Conference Proceedings.
