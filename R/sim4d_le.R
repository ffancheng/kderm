...
## ----le-------------------------------------------------------------------
if(file.exists("data/metric_le_4d_N10000_radius10.rda")){
  load("data/metric_le_4d_N10000_radius10.rda")
} else {
...
tictoc::tic()
fixden_le <- vkde(x = fn, h = NULL, vh = NULL, r = r, gridsize = gridsize, eval.points = fn, opt.method = opt.method, riem.scale = riem.scale, adj_matrix = adj_matrix)
tictoc::toc()
(H <- fixden_le$H)
...
tictoc::tic()
fle <- vkde(x = fn, h = NULL, vh = Rn, r = r, gridsize = gridsize, eval.points = fn, opt.method = opt.method, riem.scale = riem.scale, adj_matrix = adj_matrix)
tictoc::toc()
...
cormethod <- "spearman"
(cors <- c(
  cor(trueden, fle$estimate, method = cormethod),
  cor(trueden, fixden_le$estimate, method = cormethod)
))
mean(sqrt((trueden - fle$estimate)^2)) # smaller MSE
mean(sqrt((trueden - fixden_le$estimate)^2))

noutliers <- 100
sum(head(order(fle$estimate), noutliers) %in% head(order(trueden), noutliers))
sum(head(order(fixden_le$estimate), noutliers) %in% head(order(trueden), noutliers))

f <- tibble(fxy = trueden, # true densities
            fxy_dckde = fle$estimate, fxy_hdr = fixden_le$estimate
...
# p
ggsave(paste0("figures/rankplot_4d_N", N, "_", method, "_radius", radius, "_r", format(r, decimal.mark = "_"), ".png"), p, width = 10, height = 6, dpi = 300)


save(method, fixden_le, fle, train, trueden, cors,
     file = paste0("figures/compareden_4d_N", N, "_", method, "_radius", radius, "_r", format(r, decimal.mark = "_"), "_truedenk", k, ".rda"))
# save(metric_le, file = paste0("data/metric_le_4d_N10000_radius", radius, ".rda"))
