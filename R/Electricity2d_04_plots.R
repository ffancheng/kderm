...
isomapBNNtime <- microbenchmark::microbenchmark(
  {isomapbnn <- dimRed::embed(train, .method = method, knn = pars$knn, 
...
isomapANNtime <- microbenchmark::microbenchmark(
  {isomapann <- dimRed::embed(train, .method = method, knn = pars$knn, 
                              ...
                              nt = pars$nt,
                              nlinks = pars$nlinks, ef.construction = 500,
                              ndim = pars$ndim, distance = pars$distance, .mute = c("output"))}, # , distance = pars$distance
  times=ntimes, 
  unit = "s"
)
...
(isomapBNNmed <- summary(isomapBNNtime)$median) # 14.0103s
...
Y_isomap_brute <- isomapbnn@data@data %>% as.data.frame()
...
ann_table_isomapbnn <- calc_ann_table(e=isomapbnn, nn.idx=truenn)
...
ann_table_isomap <- bind_rows(ann_table_isomapbnn, ann_table_isomapnn, ann_table_isomapann, ann_table_isomapann_annoy) %>% 
  mutate(time = c(isomapBNNmed, isomapNNmed, isomapANNmed, isomapANNmed_annoy))
row.names(ann_table_isomap) <- c("Exact NN with brute-force", "Exact NN with k-d trees", "k-d trees", "Annoy")
ann_table_isomap
...
save(isomapbnn, isomapnn, isomapann, isomapann_annoy, ann_table_isomap, pars, eps, nt, isomapBNNtime, isomapNNtime, isomapANNtime, isomapANNtime_annoy, 
     # Y_isomap, Y_annisomap, Y_annisomap_annoy, 
     file = paste0(folder, 'electricityplot_', method0, "_kdtree_eps", eps, "_annoy_nt_", nt, filename, '.rda') # for k-d trees
...
lleBNNtime <- microbenchmark::microbenchmark(
  {llebnn <- dimRed::embed(train, .method = method, knn = pars$knn, 
                           annmethod = "brute-force",
                           eps = pars$eps,
                           nt = pars$nt,
                           nlinks = pars$nlinks, ef.construction = 500,
...
Y_lle_brute <- llebnn@data@data %>% as.data.frame()
...
(lleBNNmed <- summary(lleBNNtime)$median)
...
# Calculate embedding quality measures
ann_table_llebnn <- calc_ann_table(e=llebnn, nn.idx=truenn)
...
ann_table_lle <- bind_rows(ann_table_llebnn, ann_table_llenn, ann_table_lleann, ann_table_lleann_annoy) %>% 
  mutate(time = c(lleBNNmed, lleNNmed, lleANNmed, lleANNmed_annoy))
row.names(ann_table_lle) <- c("Exact NN with brute-force", "Exact NN with k-d trees", "k-d trees", "Annoy")
ann_table_lle
# M_T       M_C      LCMC       Qnx        W_n
...
save(llebnn, llenn, lleann, lleann_annoy, ann_table_lle, pars, eps, lleBNNtime, lleNNtime, lleANNtime, lleANNtime_annoy,
...
leBNNtime <- microbenchmark::microbenchmark(
  {lebnn <- dimRed::embed(train, .method = method, knn = pars$knn, 
                          annmethod = "brute-force",
                          eps = pars$eps,
                          nt = pars$nt,
                          nlinks = pars$nlinks, ef.construction = 500,
...
Y_le_brute <- lebnn@data@data %>% as.data.frame()


(leBNNmed <- summary(leBNNtime)$median)
...
# Calculate embedding quality measures
ann_table_lebnn <- calc_ann_table(e=lebnn, nn.idx=truenn)
...
ann_table_le <- bind_rows(ann_table_lebnn, ann_table_lenn, ann_table_leann, ann_table_leann_annoy) %>% 
  mutate(time = c(leBNNmed, leNNmed, leANNmed, leANNmed_annoy))
row.names(ann_table_le) <- c("Exact NN with brute-force", "Exact NN with k-d trees", "k-d trees", "Annoy")
ann_table_le
# M_T       M_C      LCMC       Qnx        W_n
# Exact NN  0.9692678 0.9105383 0.1919331 0.1974306 0.02703690
# k-d trees 0.9330203 0.8194326 0.1094104 0.1149079 0.05755084
...
save(lebnn, lenn, leann, leann_annoy, ann_table_le, pars, eps, leBNNtime, leNNtime, leANNtime, leANNtime_annoy,
     # Y_lle, Y_annlle, Y_annlle_annoy, 
     file = paste0(folder, 'electricityplot_', method0, "_kdtree_eps", eps, "_annoy_nt_", nt, filename, '.rda') # for k-d trees
     # file = paste0(folder, 'electricityplot/electricityplot_', method0, "_", annmethod, "_nt", nt, "_", filename, '.rda') # for Annoy
)
...
hlleBNNtime <- microbenchmark::microbenchmark(
  {hllebnn <- dimRed::embed(train, .method = method, knn = pars$knn, 
                            annmethod = "brute-force",
                            eps = pars$eps,
                            nt = pars$nt,
                            nlinks = pars$nlinks, ef.construction = 500,
...
Y_hlle_brute <- hllebnn@data@data %>% as.data.frame()
...
(hlleBNNmed <- summary(hlleBNNtime)$median)
...
# plot(Y_hlle)
# plot(Y_annhlle)
all.equal(Y_hlle, Y_annhlle)

# Calculate embedding quality measures
ann_table_hllebnn <- calc_ann_table(e=hllebnn, nn.idx=truenn)
...
ann_table_hlle <- bind_rows(ann_table_hllebnn, ann_table_hllenn, ann_table_hlleann, ann_table_hlleann_annoy) %>% 
  mutate(time = c(hlleBNNmed, hlleNNmed, hlleANNmed, hlleANNmed_annoy))
row.names(ann_table_hlle) <- c("Exact NN with brute-force", "Exact NN with k-d trees", "k-d trees", "Annoy")
ann_table_hlle
# M_T       M_C      LCMC       Qnx        W_n
# Exact NN  0.9692678 0.9105383 0.1919331 0.1974306 0.02703690
# k-d trees 0.9330203 0.8194326 0.1094104 0.1149079 0.05755084
...
# k-d trees 0.001564043  0.6876028 0.1100152 0.6378675 23.53195
# Annoy     0.001594595  0.6432459 0.1739833 0.6413438 23.32004
all.equal(ann_table_hllenn, ann_table_hlleann)

save(hllebnn, hllenn, hlleann, hlleann_annoy, ann_table_hlle, pars, eps, hlleBNNtime, hlleNNtime, hlleANNtime, hlleANNtime_annoy,
...
tsneBNNtime <- microbenchmark::microbenchmark(
  {tsnebnn <- embed(train, .method = method, knn = pars$knn,
                    annmethod = "brute-force",
                    eps = pars$eps,
                    nt = pars$nt,
                    nlinks = pars$nlinks, ef.construction = 500,
...
Y_tsne_brute <- tsnebnn@data@data %>% as.data.frame()
...
(tsneBNNmed <- summary(tsneBNNtime)$median)
...
# Calculate embedding quality measures
ann_table_tsnebnn <- calc_ann_table(e=tsnebnn, nn.idx=truenn)
...
ann_table_tsne <- bind_rows(ann_table_tsnebnn, ann_table_tsnenn, ann_table_tsneann, ann_table_tsneann_annoy) %>% 
  mutate(time = c(tsneBNNmed, tsneNNmed, tsneANNmed, tsneANNmed_annoy))
row.names(ann_table_tsne) <- c("Exact NN with brute-force", "Exact NN with k-d trees", "k-d trees", "Annoy")
ann_table_tsne
# M_T       M_C       LCMC        Qnx       W_n
...
save(tsnebnn, tsnenn, tsneann, tsneann_annoy, ann_table_tsne, pars, eps, tsneBNNtime, tsneNNtime, tsneANNtime, tsneANNtime_annoy, 
...
umapBNNtime <- microbenchmark::microbenchmark(
  {umapbnn <- embed(train, .method = method, knn = pars$knn,
                    annmethod = "kdtree",
                    eps = pars$eps,
                    nt = pars$nt,
                    nlinks = pars$nlinks, ef.construction = 500,
...
Y_umap_brute <- umapbnn@data@data %>% as.data.frame()
...
(umapBNNmed <- summary(umapBNNtime)$median)
...
# Calculate embedding quality measures
ann_table_umapbnn <- calc_ann_table(e=umapbnn, nn.idx=truenn)
...
ann_table_umap <- bind_rows(ann_table_umapbnn, ann_table_umapnn, ann_table_umapann, ann_table_umapann_annoy) %>% 
  mutate(time = c(umapBNNmed, umapNNmed, umapANNmed, umapANNmed_annoy))
row.names(ann_table_umap) <- c("Exact NN with brute-force", "Exact NN with k-d trees", "k-d trees", "Annoy")
ann_table_umap
# M_T       M_C       LCMC        Qnx       W_n
# Exact NN  0.8856043 0.8939552 0.12956705 0.13506458 0.1117472
# k-d trees 0.9187908 0.8054756 0.06619799 0.07169552 0.0777555
...
save(umapbnn, umapnn, umapann, umapann_annoy, ann_table_umap, pars, eps, umapBNNtime, umapNNtime, umapANNtime, umapANNtime_annoy, 
...
rownames(trustworthiness_1id) <- c("Exact NN with brute-force", "Exact NN with k-d trees", "ANN k-d trees", "ANN Annoy")
(trustworthiness_1id <- trustworthiness_1id[-1,])
...
rownames(time_allid) <- c("Exact NN with brute-force", "Exact NN with k-d trees", "ANN k-d trees", "ANN Annoy")
time_allid
save(time_allid, file = paste0(folder, 'electricityplot_time_nt', nt, filename, '.rda'))

time_allid_brute <- time_allid[1,]
save(time_allid_brute, file = paste0(folder, 'electricityplot_time_nt', nt, "_bruteonly", filename, '.rda'))
...
## data/1id_countratio_nosmooth -> paper/figures to save pictures
cat("Finished at: "); Sys.time()
...
# for(i in 1:6) load(list.files("data/allids_count_ratio", full.names = TRUE)[i])
...
ggsave(paste0(folder, "embedding_compare_allids", filename, ".png"), emb, width=10, height=6)
...
ggsave(paste0(folder, "tod_compare_kdtreeannoy_6methods_", filename, ".png"), tod, width=10, height=6, dpi = 500)
...
ggsave(paste0(folder, "tod_compare_kdtreeannoy_6methods_", filename, "_transpose.png"), tod_t, width=8, height=11, dpi = 500)

((pt1 + labs(title = "Exact NN", x="", y="ISOMAP")) | 
    (pta1 + labs(title = "k-d trees", x="", y="")) |
    (ptaa1 + labs(title = "Annoy", x="", y=""))) + 
  plot_layout(guides = "collect") &
  theme(legend.position = 'bottom', plot.title = element_text(hjust = 0.5, face = "bold")) 
...
if(nid == 1) ids <- 1:336 # run this for single ID
...
#   theme(legend.position = 'bottom') 
# hdr_lle
# ggsave(paste0(folder, "hdr10_comparelle_", filename, ".png"), hdr_lle, width=10, height=6)
