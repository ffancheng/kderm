...
```{r}
load("swissroll/metric4methods.rda")
```
...
radius <- 4 # the bandwidth parameter, \sqrt(\elsilon), as in algorithm. Note that the radius need to be changed for different datasets, not to increase k
```
...
fxy <- get(paste0("fxy_", methods))
plot(preswissroll$den, fxy, xlab = "True density", ylab = "Density estimates")
abline(0, 1, col = "red")
}
```
...
p_lle_all <- plot_outlier(x = metric_lle, n.grid = 20, prob = prob, noutliers = N, scale = 1/20, f = flle)
...
for(i in 1:4) {
  ...
  plot(den_rank, outlier$outliers, xlab = "True outlier rankinng", ylab = "Estimated outlier ranking")
  abline(0, 1, col = "red")
}
```
...
for(i in 1:4) {
  for (j in (i+1):4) {
    ...
    plot(outlier1$outliers, outlier2$outliers, xlab = paste("Outlier rankinng from", method[i]), ylab = paste("Outlier rankinng from", method[j]))
  # abline(0, 1, col = "red")
  }
