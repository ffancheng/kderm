...
# Swiss roll mapping
u_sr <- (1.5 * pi) * (1 + 2 * co) # U(1.5pi, 4.5pi)
a <- u_sr[,1]
claytonsr <- tibble(x = a * cos(a), y = u_sr[,2], z = a * sin(a))
...
col <- rep("grey", N)
col[head(order(claytonden), n=20)] <- "red"
col[tail(order(claytonden), n=20)] <- "black"
par(mfrow=c(1,2))
...
```{r parameters}
# train <- swissroll; claytonden <- preswissroll$den
train <- claytondata
# Parameters fixed
x <- train
N <- nrow(x)
...
plot_embedding(metric_isomap) +
  labs(x = "ISO1", y = "ISO2")
plot_ellipse(metric_isomap, add = F, n.plot = 50, scales = 20,
...
p_hdr_isomap_p <- p_hdr_isomap + 
  plot_ellipse(metric_isomap, add = T, n.plot = 50, scales = 1, 
...
p_isomap <- plot_outlier(x = metric_isomap, n.grid = n.grid, prob = prob, scales = 1, ell_size = ell_size, f = fisomap)
p_isomap$p
...
p_lle <- plot_outlier(x = metric_lle, n.grid = 20, prob = prob, noutliers = 20, scales = 1, f = flle, ell_size = ell_size)
...
```{r tsne, message=FALSE, warning=FALSE, eval=FALSE}
...
p_hdr_tsne <- hdrcde::hdrscatterplot(E1, E2, kde.package = "ks", noutliers = 20)
...
p_hdr_umap <- hdrcde::hdrscatterplot(E1, E2, kde.package = "ks", noutliers = 20)
...
for(i in 2){
  ...
  cor(fxy, p_hdr_lle$densities, method = "spearman") %>% print()
}
