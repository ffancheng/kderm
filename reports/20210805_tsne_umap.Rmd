...
<!-- \begin{algorithm}[!htb]
  ...
  \STATE Constructe a local fuzzy simplicial set by setting $p_{i j}=p_{j \mid i}+p_{i \mid j} - p_{i \mid j} p_{j \mid i}$;

  \STATE Compute the distribution of distances between $y_i$ and $y_j$ as
  $$
    q_{i j}=\left(1+a\left(y_{i}-y_{j}\right)^{2 b}\right)^{-1},
  $$
  where $a$ and $b$ are found using $\text { min_dist }$ parameter such that
  $$
      q_{i j} \approx\left\{\begin{array}{ll}
  1 & \text { if } y_{i}-y_{j} \leq \text { mindist } \\
  e^{-\left(y_{i}-y_{j}\right)-\text { mindist }} & \text { if } y_{i}-y_{j} > \text { mindist };
  \end{array}\right.
  $$
  ...
  \STATE Optimize $y_i$ by minimizing the cross entropy using stochastic gradient descent with *n_epochs*.
...
```{r umap, fig.align = 'center', out.width = "80%", echo = FALSE, fig.cap = "", eval=TRUE}
...
TODO: run simulations for both dimension 2 and 12 on MNIST and check if the conclusion holds. For electricity dataset, since the goal is to find anomalies, we could choose dimension 2 for simplicity. (otherwise compute densities for high-dimensional embedding)
