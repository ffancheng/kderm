...
plot_contour(metric_tsne, n.grid = 20, scale = 1/10)
```

```{r}
p_tsne <- plot_outlier(x = metric_tsne, n.grid = 20, prob = prob, noutliers = 20, scale = 1/10)
...
head(p_tsne$outlier, n = 20)
outlier_isomap <- order(as.numeric(p_isomap$outlier))
outlier_tsne <- order(as.numeric(p_tsne$outlier))
