...
\end{equation} 
where $\pmb{H}_i$ is called the bandwidth matrix which is a $d\times d$ symmetric positive definite matrix variable at each $i$, $K(\pmb{y})$ is the kernel function, and $K_{\pmb{H}_i}(\pmb{y}) = |\pmb{H_i}|^{-1/2} K(\pmb{H_i}^{-1/2}\pmb{y})$.
When using the standard gaussian kernel, $K(\pmb{y}) = (2\pi)^{-d/2}\exp(-\frac{1}{2}\pmb{y}'\pmb{y})$, we have
\begin{equation}
\label{eq:kernel}
...
$$ 
if $(s,t) \neq (0,0)$ and $\Psi^{-1}(0,0)=p$ where $r=\sqrt{s^2 + t^2}$. Note that $r=d_g(p,q)$ if $q = \exp_p(sv+tw)$.
Then we take the partial derivatives of \$\\Psi\^{-1}(s,t)\$ with respect to \$s\$ and \$t\$ and have that
$$
...
$$ 
Then each component of the riemannian metric could be calculated as $g(A(s,t), A(s,t)), g(A(s,t), B(s,t)), g(B(s,t), B(s,t))$. We obtain that the volume density on the sphere is 
$$
...
Similarly in $S^1$, we could let $\{v, p\}$ be the basis of $\mathbb{R}^2$. Then the exponential chart induced
