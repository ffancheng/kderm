...
data.frame(cors) %>% 
  ...
  kableExtra::kbl(caption = "Correlation between true density ranking and estimated density ranking for different manifold learning embeddings.", booktabs = TRUE, digits = 3, escape = FALSE) %>%
...
For comparison, we use the same meta data in \autoref{fig:metadensity} with different mapping function, twin peak mapping in Equation \@ref(eq:twinpeak), with the corresponding 3-D data shown in the right plot of \autoref{fig:mappings}.
\begin{equation}
\label{eq:twinpeak}
\left\{ 
\begin{array}{lcl}
