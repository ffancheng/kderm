...
Given the 2-D meta data, multiple mapping functions could be applied to embed the data in a 3-D space. One of the most famous example in manifold learning is the swiss roll data, with the mapping function in \@ref(eq:swissroll). The two-dimensional meta data $(\pmb{X}_1, \pmb{X}_2)$ is transformed into the three-dimensional data $(\pmb{X}, \pmb{Y}, \pmb{Z})$, shown in the left plot of \autoref{fig:mappings}.
\begin{equation}
\label{eq:swissroll}
\left\{ 
\begin{array}{lcl}
...
```{r srcors, echo=FALSE, message=FALSE, eval=F}
load("figures/CorrelationTable_Swiss Roll_4ml_riem01.rda")
...
For comparison, we use the same meta data with different mapping function, twin peak mapping in Equation \@ref(eq:twinpeak), with the corresponding 3-D data shown in the right plot of \autoref{fig:mappings}. The four colors in both mappings represents the four gaussian kernels used to generate the meta data $(\pmb{X}_1, \pmb{X}_2)$. 
\begin{equation}
\label{eq:twinpeak}
\left\{ 
\begin{array}{lcl}
...
knitr::include_graphics("figures/Twin Peak_outliers_comparison_4ml_3cases_riem01.png")
```
...
```{r tpisomapden, fig.align = 'center', out.width = "95%", fig.cap="Scatterplot of true density and estimated density of ISOMAP embedding for KDE with both variable and fixed bandwidth. The four colors represents the four gaussian kernels in the 2-D meta data.", echo = FALSE}
knitr::include_graphics("figures/Twin Peak_density_comparison_isomap_riem01.png")
```
...
load("figures/CorrelationTable_twinpeak_4ml_riem01.rda")
col.max <- apply(cors, 2, max)
max.match <- rep(col.max, each = 2) == cors
data.frame(cors) %>% 
  mutate_if(is.numeric, format, digits = 3, nsmall = 3) %>% 
