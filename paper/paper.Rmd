...
For any tangent vector $u \in T_pM$, let $\gamma(u): t \rightarrow \exp_p(\frac{tu}{\|u\|})$ denote a curve passing through $u$, and let $w_i, i=1,\dots,d$ be Jacobi fields along $\gamma$ such that $w_i(0)=0$ for all $i=1,\dots,d$ and $\frac{Dw_1}{dt}(0)=\frac{u}{\|u\|}$. Then $\frac{Dw_1}{dt}(0)$ forms an orthonomal basis of the tangent space $T_pM$. 
...
Again, the volume density function is not defined for all $\pmb{p}$ but for points that satisfies $\pmb{p} \in B(\pmb{y}_i,r)$. 


[TODO: Further fix the approximation induced by the estimated d_g so that f will integral to 1.]

As for the geodesic distance $d_g$, 
