...
A \(d\)-dimensional (topological) \emph{manifold} \(M\) is a connected Hausdorff space \((M, \mathcal{T}_M)\) where the neighborhood \(U\) for each point \(p\) is homeomorphic to an open subset \(V\) of the Euclidean space \(\mathbb{R}^d\). Such a homeomorphism \(\varphi: U \rightarrow V\) and \(U\) gives a (coordinate) \emph{chart}, denoted as \((U, \varphi)\), with the corresponding local coordinates \((x_1(p),\dots, x_d(p)) := \varphi(p)\). A collection of charts \(\{U_\alpha, \varphi_\alpha\}\) ranging over the manifold \(M\) is called an \emph{atlas}, denoted as \(\mathcal{A}\).
...
\]
A \emph{tangent vector} to \(M\) at \(p\) is any equivalent class of the differentiable curves initialized at \(p\).
The set of all tangent vectors at \(p\) defines the \emph{tangent space} of \(M\) at \(p\), denoted as \(T_pM\). The tangent space is vector space of dimension \(d\), equal to the dimension of \(M\), and it does not depend on the charts \(\varphi\) locally at \(p\). The collection of all tangent spaces defines the \emph{tangent bundle} \(TM = \cup_{p \in M}T_pM\).
Tangent vectors can also be seen as the directionnal derivatives at \(p\). For a given coordinate chart \(\varphi=(x_1,\dots,x_d)\), the tangent vectors defining partial derivatives are denoted as \(\frac{\partial}{\partial x_1}(p),\dots,\frac{\partial}{\partial x_d}(p)\), which define a basis of the tangent space \(T_pM\).
...
Define the directional derivative \(D_v\) in the direction of \(v\) as
\[
\forall f \in C^\infty(M): D_v(f)
\]

\hypertarget{riemannian-metrics}{%
\subsection{Riemannian metrics}\label{riemannian-metrics}}

Let \((\mathcal{M}, g)\) be a \(d\)-dimensional Riemannian manifold without boundary. Denote \(d_g\) the Riemannian distance. Assume that \((\mathcal{M}, g)\) is complete, i.e.~\((\mathcal{M}, d_g)\) is a complete metric space, and that the injectivity radius of \((\mathcal{M}, g)\), \(inj_g\mathcal{M}\), is strictly positive.

Denote \(B_s(p)\) the normal ball in \((\mathcal{M}, g)\) centered at point \(p\) with raidus \(s\).

\(d_g\) is induced by \(g\) and by the injectivity radius of \((\mathcal{M}, g)\).
...
embedding pushforward metric \autocite{Perrault-Joncas2013-pq}
...
\hypertarget{isometry}{%
\subsection{Isometry}\label{isometry}}
