...
One limitation of the kernel density estimator is that the density can be estimated only at points where data have been observed since the estimator requires the Riemannian \(\pmb{H}_j\). To estimate the density at points that do not belong to the sampled points, any smoothed average of nearest neighbors can be used instead.While density estimation on manifolds may be used in a number of ways, a particular downstream task that we will focus on will be anomaly detection in \ref{alg:kdeanomaly}. We note that the particular downstream task that we are interested in is anomaly detection for which only the density estimates at observed sample points are required, since anomalies are identified as the points with lowest density.
