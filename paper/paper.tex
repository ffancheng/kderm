...
The bandwidth is varied depending on either the location of the sample points \autocite[sample smoothing estimator;][]{Terrell1992-ut} or that of the estimated points \autocite[balloon estimator;][]{Terrell1992-ut}. In this paper, the densities are estimated at the sample points themselves, so we only need to consider the case where the bandwidth changes for each sample point and will refer to this as the \emph{variable/adaptive kernel density estimation} \autocite[VKDE; Section 6.6 of][]{Scott2015-vl} unless otherwise stated.
