...
Tangent vectors can also be seen as the directional derivatives at \(p\). For a given coordinate chart \(\varphi=(x_1,\dots,x_d)\), the tangent vectors defining partial derivatives are denoted as \(\frac{\partial}{\partial x_1}(p),\dots,\frac{\partial}{\partial x_d}(p)\), which define a \emph{basis} of the tangent space \(T_pM\).
