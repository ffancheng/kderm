...
\hypersetup{
            pdftitle={Distortion-corrected kernel density estimate for Riemannian manifolds},
...
\title{Distortion-corrected kernel density estimate for Riemannian manifolds}
...
\hypertarget{vkde}{%
\section{Kernel density estimate on Riemannian manifold embeddings}\label{vkde}}
...
Notice that this representation is not necessarily isometry to \(M\), and the underlying manifold is usually unknown in real data sets. Then the density estimator in Equation \eqref{eq:denriem} proposed by \textcite{Pelletier2005-vu} no longer applies because of the unknown geodesic distance \(d_g\) and the volume density function \(\theta_{\pmb{X}_i}(\pmb{p})\).
Motivated on the estimator in \eqref{eq:denriem}, we propose a distortion-corrected kernel density estimator (DC-KDE) of \(f\) with the manifold embedding \(y_i, i = 1, 2, \dots, N\) to be
\begin{equation}
